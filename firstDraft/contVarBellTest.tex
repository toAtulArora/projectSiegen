...
We propose a new test for local realism based on correlation measurements of continuum valued functions of positions and momenta, known as modular variables. 
...
tests of quantum mechanics as opposed to theories conforming to common sense. Since 
...
Finally, in section \ref{physicalImplementation} we show how the entire test can be implemented in a double grating setup \cite{Gneiting13}. Grating techniques have been used to experimentally demonstrate quantum matter waves \cite{GratingExp}. %as a powerful experimental demonstration of the quantum matter wave exhibited by massive particles \cite{GratingExp}.
...
Eigenvalues of $\hat A_i$, $|a_i|\leq 1$ which for bounded observables can be achieved trivially by re-scaling. An example of this is the parity operator which was used in \cite{Parity98}.\\
...
It is well recognized and can be shown that there is a tight relation between non-locality and non-commutativity of operators.
The violation, therefore, occurs for choices of settings whose corresponding observables do not commute. In our construction we can illustrate how the non-commutativity between position and momentum, $[\hat q,\hat p]=i\hbar$, account for the violation. This would be regarded as a further illustration that our approach provides a relevant test in phase space. We show that in our case $[\hat X(\theta),\hat X(\theta')]\neq 0$.
...
The test can be implemented in a quantum interferometric setups using grating technique to create a multi-component superposition states as is used in matter wave experiments for instance. 
