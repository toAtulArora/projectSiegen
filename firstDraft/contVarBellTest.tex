...
$p_{mod}\in[0,h/L)$. If we further consider, say $\cos(p_{mod}L/\hbar)$,
then we get precisely the bound needed for the CHSH inequality. We
chose the constant factors so that for $p_{mod}=h/L$, the $\cos$
function also completes a period. Note that $\cos(p_{mod}L/\hbar)=\cos(pL/\hbar)=\frac{e^{ipL/\hbar}+e^{-ipL/\hbar}}{2}$.
...
that $\hat{X}$ is a function of $\hat{p}_{mod}$. It is thus natural
to expect $\hat{Z}$ to be a function of $\hat{x}_{mod}$. From it's
action, we also note that $\hat{Z}$ must differentiate between spatial
wavefunctions $\left\langle x|\varphi\right\rangle $ and $\left\langle x-L|\varphi\right\rangle $.
Thus the appropriate operator must be $\hat{x}_{mod2L}$ and not $\hat{x}_{modL}$.
This is supported by the fact that $[\hat{p}_{mod},\hat{x}_{modL}]=0$.
Now since the action of $\hat{Z}$ must work for arbitrary $\left|\varphi\right\rangle $
and $N$, we define 
\[
\hat{Z}\equiv Z(\hat{x}_{mod2L})
\]
where $Z$ is some appropriate function, so that $\hat{Z}\left|x\right\rangle =Z(\hat{x}_{mod2L})\left|x\right\rangle =Z(x_{mod2L})\left|x\right\rangle $
. From the constraints on $\hat{Z}$, we conclude that for consistency,
we must define%
\footnote{$\hat{x}_{mod2L}$ is well defined, see \subref{c6}%
} 
\[
...
that $Z(x)=-Z(x+L)$ (omitting the $\mod2L$). Similarly one can evaluate
$W_{2}$ and obtain 
\[
...
that the source of violation essentially hinges on $[\hat{x}_{mod2L},\hat{p}_{modL}]\neq0$.
...
The question is why did $\hat{Z}=Z(\hat{x}_{mod2L})$ appear in the
analysis. The cause of this asymmetry hinges on the preferential treatment
of position space. We could have constructed states of the form $\left|\psi_{0}\right\rangle =\sum_{n}\left|x+nd\right\rangle $
...
is resolved by $Z(\hat{x}_{mod2L})$ which causes a larger set of
physically realizable states to become admissible.
...
\subsubsection{Action of $\hat{x}_{mod2L}$ can be defined explicitly\label{sub:c6}}

Proof: $\hat{x}_{mod2L}\equiv\int dx'x'_{mod2L}\left|x'\right\rangle \left\langle x'\right|$.
...
we have $\hat{x}_{mod2L}\left|c\right\rangle =\int_{0}^{2L}d\bar{x}c_{\bar{x}}\bar{x}\left|\bar{x}\right\rangle $. 
