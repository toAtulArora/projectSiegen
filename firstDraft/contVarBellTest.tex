...
\date{May-July 2015}
\begin{abstract}
Local realism tests in phase space that fulfill the explicit assumptions underling Bell's theorem is of fundamental significant.
We construct a CHSH-Bell inequality in terms of continuum valued function of position and momentum variables, known as modular variables,  which are bounded in phase space. The associate inequality hold for any state described by positive Wigner function. This conform with Bell conjecture that positive Wigner function serving as a valid probability distribution does not reveal nonlocality. We construct a class of entangled states leading to the violation of the inequality, and thus truly demonstrate nonlocality in phase space. The states can be realized in interferometric setup though grating technique. The nonlocality is verified from spatial correlation data detected from the screens.
\end{abstract}
\maketitle
...
It is, however, more tempting to formulate analogous tests for truly spatial degree of freedom and test the nonlocality in phase space as a natural extension to macroscopic local realism tests [ ]. This come along with the original EPR argument which uses phase space descriptions to better address the locality problem of quantum mechanics in such manifold. 
Moreover, phase space is a natural concept in classical theory and it is equivalent to the state space.   The outstanding challenge is to come up with a scheme for local realism test which verify or disprove quantum mechanics predictions  with most ``classical-like" measurements and operations.

The paper is organized as follows.
In Sec. we introduce our framework for Bell nonlocality test aiming to use most classical-like variables and measurements, and thus pave the way towards macroscopic test of local realism.  For our purpose, therefore, we use continuous modular variables, characterized by continuum spectral eigenvalues scaled within a limited range. These variables can be clearly demonstrated in Multi-slit interfrometric setup. In Sec. We construct the CV Bell operator of CHSH form from modular variables. In Sec. we proceed with identifying the relevant entangled state leading to the violation of the Bell inequality. Finally, in Sec. we propose a possible implementation of the desired entangled states




\section{Framework for macroscopic Bell test}
Bell argued that a positive Wigner function does not lead to violation of the inequalities derived from
local hidden variable models. Because, positive Wigner functions are valid probability distributions over local \emph{hidden} phase space coordinates.  In what follow we develop an approach which comply with above statement.
The central 
problem here is to construct a Bell-operator expressed in terms of  suitable
local continuous variable operators $\hat A$.  A general form of CHSH expression of such a 
Bell-operator reads
\[
\label{Bellmodular}
\hat {\mathcal{B}} \equiv \hat A_1\otimes (\hat A_2+\hat A_2') +\hat A_1'\otimes(\hat A_2-\hat A_2')
\]
In order to apply the CHSH criterion we need to use variables which take on a limited range of values to be able to determine a well-defined classical bound. We therefore aim to construct a class of bounded observables of continuous variable (CV) systems which have the following properties. Using proper rescaling for corresponding quantum eigenvalues of the observable we have \[
|a_j|\leq 1
\]
One obvious example is parity operator $\Pi=(-1)^{a^\dag a}$. This is enough to construct a valid Bell operator with a well-defined classical bound. We however demand of an extra constraint required for probing Nonlocality in phase space. We impose that the observable $\hat A$ corresponds to a bounded c-number function in phase space using Wigner-Weyl correspondence ($q\leftrightarrow \hat q$, $p\leftrightarrow \hat p$). That is, 
\[
\mathcal{W}_{\hat A}(q,p) \equiv \int dq' e^{ipq'}\bra{q-\dfrac{q'}{2}}\hat A \ket{q+\dfrac{q'}{2}}\leq 1
\] 
Accordingly, the inequality should hold for any state described by valid (positive) probability distribution.

\begin{align*}
\mathcal{W}_{\hat {\mathcal{B}}}(q,p)=&\mathcal{W}_{\hat A_1}(q,p)[\mathcal{W}_{\hat A_2}(q,p)+\mathcal{W}_{\hat A_2'}(q,p)] \\&+\mathcal{W}_{\hat A'_1}(q,p)[\mathcal{W}_{\hat A_2}(q,p)-\mathcal{W}_{\hat A_2'}(q,p)]\leq 2
\end{align*}

Therefore,
\begin{equation}
\mean{\hat{\mathcal{B}}}=\int W_{\hat {\rho}}(q,p)\mathcal{W}_{\hat {\mathcal{B}}}(q,p)dq dp\leq 2
\end{equation}
where $W_{\hat{\rho}}$ is the wigner function corresponding to $\hat {\rho}$ given by
\[
W_{\hat {\rho}} = \frac{\mathcal{W}_{\hat {\rho}}}{2\pi \hbar}
\]
 We that we ascertain that any state described by positive definite Wigner function, that is compatible with local hidden phase space variable, satisfy above inequality.  leading to the violation of this version of Bell inequality must be characterized by negative Wigner function deviating from proper probability distribution in hidden phase space coordinates. That's why,  John Bell remarked that as EPR state described by positive Wigner function is compatible with a local hidden variable model, and thus it does not demonstrate nonlocality. This claim can be justified in the sense of above.
...
One particular class of bounded observable can be constructed from the quantum mechanical phase space translation operator\footnote{Here $-L/\hbar$ causes a displacement by $L$.}, $e^{-i\hat p L/\hbar}$. This observable is not hermitian and therefore it does not correspond to an observable. We define
\begin{align}
\begin{split}
  \label{translation}
...
\end{split}
\end{align}
which is not only explicitly hermitian, it's also bounded by $1$. There're three relavent properties. First, from equation \ref{translation}, the action of $\hat X$ is obvious. This entails that spatial symmetry must be harnessed to construct its (approximate) eigenstates. Second, when $\hat X$ is operated on $\ket{p}$, then only the modular part of $p$ is relevant to the value of the operator. Thus we may define 
\[
...
\[
e^{i\hat{p}a}e^{i\hat{q}b}=e^{i\hbar ab}e^{i\hat{q}b}e^{i\hat{p}a}
\]
using $[q,p]=i\hbar$. For appropriate choices of $a,b$, the translation operators can be made to commute or anti-commute. In the former case, it means one can simultaneously measure modular position and momentum (which is in stark contrast with $\hat x$ and $\hat p$ measurements) and in the latter case, one can define pauli matrix like commutation.
...
We start with constructing continuous variable kets\footnote{We emphasise that the kets we construct, will not be a countable superposition
of eigenstates of $p$ or $x$ since these are highly idealized and
strictly, not even a part of the Hilbert space.%
} that satisfy the same orthogonality and mutual relations as $\left|0\right\rangle ,\left|1\right\rangle ,\left|+\right\rangle ,\left|-\right\rangle $.\footnote{Defn: $\left|0\right\rangle ,\,\left|1\right\rangle $ are s.t. $\hat{z}\left|0\right\rangle =\left|0\right\rangle $
and $\hat{z}\left|1\right\rangle =-\left|1\right\rangle $, where $\hat x, \hat y, \hat z$ refer to Pauli matrices.%
} We then construct the relevant operators and finally evaluate $\mean {\hat{\mathcal{B}}} $ to show the violation. 
To construct the states, we start with defining $L\equiv$some length scale, $N\equiv$number of slits. Consider a state $\left|\varphi\right\rangle $ symmetric about $x=L/2$, with spread $\ll L$. Formally, $\left|\varphi\right\rangle \equiv\int dq\left\langle q-L/2|\varphi'\right\rangle \left|q\right\rangle $ where $\left|\varphi'\right\rangle $ s.t. (a) $\left\langle q|\varphi'\right\rangle =\left\langle -q|\varphi'\right\rangle $ and (b) it's spread is $\ll L$.
Next, we define $\left|\varphi_{n}\right\rangle \equiv\int dq\left\langle q-nL|\varphi\right\rangle \left|q\right\rangle $ and $I_{N}=\{-\frac{N}{2},\dots,\frac{N}{2}-1,\frac{N}{2}-1\}$, defined for even $N$ only (for simplicity). Finally we define the following to be analogues of $\ket{0}$ and $\ket{1}$;
...
\left|\psi_{-}\right\rangle \equiv\frac{\left|\psi_{0}\right\rangle -\left|\psi_{1}\right\rangle }{\sqrt{2}}
\]
These states were constructed with a partial translational symmetry which is appropriate to the bounded hermitian operator $\hat X$ discussed earlier. In the limit $N\to\infty$, we have
\begin{eqnarray*}
\hat{X}\left|\psi_{+}\right\rangle  & = & +\left|\psi_{+}\right\rangle \\
\hat{X}\left|\psi_{-}\right\rangle  & = & -\left|\psi_{-}\right\rangle 
\end{eqnarray*}
just as was with $\hat x$ and $\ket{\pm}$. In general however, the relation doesn't hold. Note that definition of $\left|\psi_{\pm}\right\rangle $ justifies the word `slit' in the definition of $N$, as is clear from the illustration. This construction also preserves the orthogonality relations from the spin case; $\left\langle \psi_{0}|\psi_{1}\right\rangle =0,\,\implies\left\langle \psi_{+}|\psi_{-}\right\rangle =0$. Analogously, we construct the entangled state 
 \[
\ket{\Psi} \equiv\frac{ \ket{\psi_{+}}_1\ket{\psi_{-}}_2 - \ket{\psi_{-}}_1\ket{\psi_{+}}_2 }{\sqrt{2}}
\]
Explicitly, consider: (a) The first qubit is with Alic and the second with Bob. (b) Alice and Bob apply local unitaries $\hat U(\alpha)\otimes \mathbb I$ and $\mathbb I \otimes \hat U(\beta)$ respectively and then both measure $\hat X$. (c) Alice can choose $\alpha \in \{\alpha_1=0,\alpha_2=-\pi/2\}$ and Bob can choose $\beta \in \{\beta_1 = 3\pi/8,\beta_2 =5\pi/4\}$.\footnote{Obviously there's a certain freedom in choosing $\alpha_i$ and $\beta_i$ but that's irrelevant here}

Effectively, this amounts to defining \begin{align*}
\hat A_1 &\equiv \hat U^{\dagger}(\alpha_1) \hat X \hat U(\alpha_1),
&\hat A'_1 &\equiv \hat U^{\dagger}(\alpha_2) \hat X \hat U(\alpha_2) \\
\hat A_2 &\equiv \hat U^{\dagger}(\beta_1) \hat X \hat U(\beta_1), 
&\hat A'_2 &\equiv \hat U^{\dagger}(\beta_2) \hat X \hat U(\beta_2)
\end{align*}
The unitary $\hat U$ was introduced here to implement measurement along different directions in a plane, as is the case with spins. To measure along different directions, within say the $x-y$ plane, one simply needs to rotate by appropriate angles using $\hat u(\phi) \equiv e^{i\hat{z}\phi/2}$. Correspondingly, here we define $\hat U$ by it's action on the relevant state. $\hat{U}(\phi)$ is defined to be s.t. 
\begin{eqnarray*}
...
\end{eqnarray*}
This can achieved in principle using alternate glass slabs for photons, to adjust the relative phase as desired, since $\left|\psi_{0}\right\rangle $ and $\left|\psi_{1}\right\rangle $ are spatially separated. Alternatively, we could also define $\hat{U}(\phi)$ in its analytic form. First we claim that $\exists$ an operator $\hat{Z}$ s.t. it satisfies
$\hat{Z}\left|\psi_{0}\right\rangle =\left|\psi_{0}\right\rangle $ and $\hat{Z}\left|\psi_{1}\right\rangle =-\left|\psi_{1}\right\rangle $. Before explicitly defining $\hat{Z}$, we observe that given this operator, we can naturally define the analytic form of $\hat{U}(\phi)$. 
\[
\hat{U}(\phi)\equiv e^{i\hat{Z}\phi/2}
\]
in close analogy with the spin case. It is suggestive from this form that $\hat{Z}$ must play the role of $\hat{z}$. We know that $\hat{X}$ is a function of $\hat{p}_{\text{mod}}$. It is thus natural to expect $\hat{Z}$ to be a function of $\hat{q}_{\text{mod}}$. From it's action, we also note that $\hat{Z}$ must differentiate between spatial wavefunctions $\left\langle q|\varphi\right\rangle $ and $\left\langle q-L|\varphi\right\rangle $. Thus the appropriate operator must be $\hat{q}_{\text{mod}2L}$ and not $\hat{q}_{\text{mod}L}$. This is supported by the fact that $[\hat{p}_{\text{mod}},\hat{q}_{\text{mod}L}]=0$. Now since the action of $\hat{Z}$ must work for arbitrary $\left|\varphi\right\rangle $ and $N$, we define
 \[
\hat{Z}\equiv Z(\hat{q}_{\text{mod}2L})
\]
where $Z$ is some appropriate function, so that $\hat{Z}\left|q\right\rangle =Z(\hat{q}_{\text{mod}2L})\left|q\right\rangle =Z(q_{\text{mod}2L})\left|q\right\rangle $. From the constraints on $\hat{Z}$, we conclude that for consistency,
we must define\footnote{$\hat{x}_{\text{mod}2L}$ is well defined, see \subref{c6}}
\[
...
$\left\langle \mathcal {\hat B} \right\rangle $ as a function of $N$. To get a violation,
we need merely $7$ slits, while with $50$ we are almost at $2\sqrt{2}$.\label{chshViolation}}
\end{figure}


Having defined $\Psi$, $\hat A_i$ and $\hat A'_i$, we are now in a position to evaluate $\mean{\mathcal {\hat B}}$. We essentially need terms of the form $\left\langle \hat{U}^{\dagger}(\alpha_{i})\hat{X}\hat{U}(\alpha_{i})\otimes\hat{U}^{\dagger}(\beta_{i})\hat{X}\hat{U}(\beta_{i})\right\rangle $. If we assume $N\to\infty$, these can be evaluated easily to be the same as the spin case; $-\cos(\alpha_{i}-\beta_{i})$ (see \subref{c3}). Substituting for the angles as defined, we obtain $\left\langle \mathcal {\hat B} \right\rangle =2\sqrt{2}$; the maximum violation, in a completely different setup. If $N$ is not assumed to be arbitrarily large, then it can be shown that (see \subref{c5})

\begin{eqnarray*}
\left\langle \hat{U}^{\dagger}(\alpha_{i})\hat{X}\hat{U}(\alpha_{i})\otimes\hat{U}^{\dagger}(\beta_{i})\hat{X}\hat{U}(\beta_{i})\right\rangle  & =\\
 & -\left(\frac{N-1}{N}\right)^{2} & \cos(\alpha_{i}-\beta_{i})
\end{eqnarray*}
Thus obviously then, $\left\langle \mathcal {\hat B} \right\rangle =\left(\frac{N-1}{N}\right)^{2}2\sqrt{2}$. If we ask for the smallest $N$ s.t. $\left\langle \mathcal {\hat B}\right\rangle >2$, we get $N\ge7$; see figure \ref{chshViolation}.
