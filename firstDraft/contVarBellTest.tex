...
\begin{align}
\label{Bellmodular}
\hat {\mathcal{B}}= \hat A_1\otimes &(\hat A_2+\hat A_2') +\hat A_1'\otimes(\hat A_2-\hat A_2')
\end{align}
...
|a_j|\leq 1
\end{equation}
One obvious example is parity operator $\Pi=(-1)^{a^\dag a}$. This is enough to construct a valid Bell operator with a well-defined classical bound. We however demand of an extra constraint required for proper probing Nonlocality in phase space. We impose that the observable $\hat A$ corresponds to a bounded c-number function in phase space using Wigner-Weyl correspondence. That is, 
\begin{equation}
\mathcal{W}_{\hat A}(q,p)=\dfrac{1}{2\pi\hbar}\int dq' e^{ipq'}\bra{q-\dfrac{q'}{2}}\hat A \ket{q+\dfrac{q'}{2}}\leq 1
\end{equation} 
Accordingly, the inequality should hold for any state described by valid (positive) probability distribution.

\begin{align}
\mathcal{W}_{\hat {\mathcal{B}}}(q,p)=&\mathcal{W}_{A_1}(q,p)[\mathcal{W}_{A_2}(q,p)+\mathcal{W}_{A_2'}(q,p)] \\&+\mathcal{W}_{A'_1}(q,p)[\mathcal{W}_{A_2}(q,p)-\mathcal{W}_{A_2'}(q,p)]\leq 2 \nonumber
\end{align}

Therefore,
\begin{equation}
\mean{\hat{\mathcal{B}}}=\int \mathcal{W}_{\hat\rho}(q,p)\mathcal{W}_{\hat {\mathcal{B}}}(q,p)dq dp\leq 2
\end{equation}
