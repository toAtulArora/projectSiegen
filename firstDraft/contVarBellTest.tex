...
To construct the states, consider a sharp state $\varphi(q) = \braket{q|\varphi}$ symmetric about the position $q=L/2$, where $L\equiv$some length scale. Further $\varphi_n(q)\equiv \varphi(q-nL)$ and $I_{N} \equiv \{-\frac{N}{2},-\frac{N}{2}+1,\dots,\frac{N}{2}-2,\frac{N}{2}-1\}$, defined for even $N$ only (for simplicity); $N\equiv$number of `slits'. Now we can define the following analogues of $\ket{0}$ and $\ket{1}$;
...
\bra{\psi_-} \hat X \ket{\psi_-} &= -\frac{N-1}{N}
\end{align*}
 %\begin{eqnarray*}\hat{X}\left|\psi_{+}\right\rangle  & = & +\left|\psi_{+}\right\rangle \\ \hat{X}\left|\psi_{-}\right\rangle  & = & -\left|\psi_{-}\right\rangle \end{eqnarray*} just as was with $\hat x$ and $\ket{\pm}$. In general however, the relation doesn't hold. 
Note that the definition of $\left|\psi_{\pm}\right\rangle $ justifies the word `slit' for $N$. %This construction also preserves the orthogonality relations from the spin case;$\left\langle \psi_{0}|\psi_{1}\right\rangle =0,\,\implies\left\langle \psi_{+}|\psi_{-}\right\rangle =0$. 
Before proceeding further, we introduce a unitary operator $\hat U$ to implement measurement `along different directions'. Typically, to measure along a vector in the $x-y$ plane, one may simply rotate by an appropriate angle using $\hat u(\phi) \equiv e^{i\hat{\sigma}_z\phi/2}$ and then measure $\hat{\sigma}_x$. Correspondingly, here we define $\hat U$ to be s.t. 
\begin{eqnarray*}
...
%\begin{align*}   \hat A_1 &\equiv \hat X (\phi), &\hat A'_1 &\equiv \hat X (\phi') \\  \hat A_2 &\equiv \hat X (\theta), &\hat A_2 &\equiv \hat X (\theta')\end{align*}
...
Having defined $\Psi$, $\hat A_i$ and $\hat A'_i$, we are now in a position to evaluate $\mean{\mathcal {\hat B}}$. We essentially need terms like $ \mean{ \hat X(\phi) \otimes \hat X(\theta)}$, where $\hat X (\theta) \equiv \hat U^{\dagger}(\theta) \hat X \hat U(\theta)$. %$\left\langle\hat{U}^{\dagger}(\phi)\hat{X}\hat{U}(\phi)\otimes\hat{U}^{\dagger}(\theta)\hat{X}\hat{U}(\theta)\right\rangle $. 

If we assume $N\to\infty$, these can be evaluated easily to be the same as the spin case; $-\cos(\phi-\theta)$ (see \subref{c3}). Substituting for the angles\footnote{One possible definition: $\theta=0,\theta'=-\pi/2;\,\phi=3\pi/8,\phi'=5\pi/4$} we obtain $\mean{\mathcal {\hat B}} =2\sqrt{2}$; the maximum violation, in a completely different setup. If $N$ is not assumed to be arbitrarily large, then it can be shown that (see \subref{c5})
\[
...
everything after the grating, then the setup is expected to produce a $\left|\psi_{+}\right\rangle $ state, just after the grating. To produce the desired entangled state, we start with two entangled photons, such that their polarization state can be expressed as $\ket{\chi} \equiv \frac{ \ket{H}_1\ket{V}_2 -\ket{V}_1\ket{H}_2 }{\sqrt{2}}$. Their spatial description (along $x$-axis) is initially assumed to be $\ket{\zeta} = \ket{\gamma}_1 \ket{\gamma}_2$.
...
the state becomes (see \subref{c8}) % \[\left|\chi_{\text{45}}\right\rangle \left|\zeta_{\text{birefringent}}\right\rangle \propto\left|H'H'\right\rangle \left(\frac{\left|\psi_{+}\psi_{-}\right\rangle -\left|\psi_{-}\psi_{+}\right\rangle }{\sqrt{2}}\right)\]
\[
\ket{\chi_{45}}\ket{\Psi} = \ket{H'}_1\ket{H'}_2 \frac{\ket{\psi_+}_1\ket{\psi_-}_2 - \ket{\psi_-}_1\ket{\psi_+}_2}{\sqrt 2}
\]
