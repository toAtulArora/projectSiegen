...
\hat{X}\left|\psi_{-}\right\rangle  & = & -\left|\psi_{-}\right\rangle 
\end{eqnarray*}
just as was with $\hat x$. In general however, the relation doesn't hold. Note that definition of $\left|\psi_{\pm}\right\rangle $ justifies the word `slit' in the definition of $N$, as is clear from the illustration. The construction preserves the orthogonality relations from the spin case; $\left\langle \psi_{0}|\psi_{1}\right\rangle =0,\,\implies\left\langle \psi_{+}|\psi_{-}\right\rangle =0$. Analogous to the spin case, we construct the entangled state 
 \[
\left|\Psi\right\rangle \equiv\frac{\left|\psi_{+}\psi_{-}\right\rangle -\left|\psi_{-}\psi_{+}\right\rangle }{\sqrt{2}}
\]
