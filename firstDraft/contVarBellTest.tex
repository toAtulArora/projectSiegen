...
We begin with constructing continuous variable kets\footnote{We emphasise that the kets we construct, will not be a countable superposition
of eigenstates of $p$ or $x$ since these are highly idealized and
strictly, not even a part of the Hilbert space.%
} that satisfy the same orthogonality and mutual relations as $\left|0\right\rangle ,\left|1\right\rangle ,\left|+\right\rangle ,\left|-\right\rangle $.\footnote{Defn: $\left|0\right\rangle ,\,\left|1\right\rangle $ are s.t. $\hat{\sigma}_z\left|0\right\rangle =\left|0\right\rangle $
...
To construct the states, consider a sharp state $\left|\varphi\right\rangle $ symmetric about $x=L/2$. Formally then, $\left|\varphi\right\rangle \equiv\int dq\left\langle q-L/2|\varphi'\right\rangle \left|q\right\rangle $ where $\left|\varphi'\right\rangle $ s.t. (a) $\left\langle q|\varphi'\right\rangle =\left\langle -q|\varphi'\right\rangle $ and (b) it's spread is $\ll L$,  where $L\equiv$some length scale. Next, we define $\left|\varphi_{n}\right\rangle \equiv\int dq\left\langle q-nL|\varphi\right\rangle \left|q\right\rangle $ and $I_{N}=\{-\frac{N}{2},\dots,\frac{N}{2}-1,\frac{N}{2}-1\}$, defined for even $N$ only (for simplicity); $N\equiv$number of `slits'. Now we can define the following to be analogues of $\ket{0}$ and $\ket{1}$;
...
\bra{\psi_-} \hat X \ket{\psi_-} &= -\frac{N-1}{N}
\end{align*}
where in the limit $N\to\infty$, we approach the familiar $\pm 1$ values. %\begin{eqnarray*}\hat{X}\left|\psi_{+}\right\rangle  & = & +\left|\psi_{+}\right\rangle \\ \hat{X}\left|\psi_{-}\right\rangle  & = & -\left|\psi_{-}\right\rangle \end{eqnarray*} just as was with $\hat x$ and $\ket{\pm}$. In general however, the relation doesn't hold. 
Note that the definition of $\left|\psi_{\pm}\right\rangle $ justifies the word `slit' for $N$, as is clear from the illustration. %This construction also preserves the orthogonality relations from the spin case;$\left\langle \psi_{0}|\psi_{1}\right\rangle =0,\,\implies\left\langle \psi_{+}|\psi_{-}\right\rangle =0$. 
Finally, we construct the much awaited entangled state which will yield a violation.
 \[
\ket{\Psi} \equiv\frac{ \ket{\psi_{+}}_1\ket{\psi_{-}}_2 - \ket{\psi_{-}}_1\ket{\psi_{+}}_2 }{\sqrt{2}}
\]
Before proceeding further, we introduce a unitary operator $\hat U$ to implement measurement `along different directions'. Typically, to measure along a vector in the $x-y$ plane, one may simply rotate by an appropriate angle using $\hat u(\phi) \equiv e^{i\hat{z}\phi/2}$ and then measure $\hat{\sigma}_x$. Correspondingly, here we define $\hat U$ to be s.t. 
\begin{eqnarray*}
...
We now describe explicitly, how all these fit with the framework laid out earlier. Consider (a) The first qubit is with Alice and the second with Bob. (b) Alice and Bob apply local unitaries, say $\hat U(\phi)\otimes \mathbb I$ and $\mathbb I \otimes \hat U(\theta)$ respectively and then both measure $\hat X$. (c) Alice can choose between $\phi$ and $\phi'$ and Bob can choose between $\theta$ and $\theta'$. %{\theta_1 = 3\pi/8,\theta_2 =5\pi/4\}$. \footnote{Obviously there's a certain freedom in choosing $\phi_i$ and $\theta_i$ but that's irrelevant here}

Effectively, this amounts to defining 
\begin{align*}
  \hat A_1 &\equiv \hat X (\phi), &\hat A'_1 &\equiv \hat X (\phi') \\
  \hat A_2 &\equiv \hat X (\theta), &\hat A_2 &\equiv \hat X (\theta')
\end{align*}
where $\hat X (\theta) \equiv \hat U^{\dagger}(\theta) \hat X \hat U(\theta)$.
%\begin{align*}
%\hat A_1 &\equiv \hat U^{\dagger}(\phi) \hat X \hat U(\phi),
%&\hat A'_1 &\equiv \hat U^{\dagger}(\phi') \hat X \hat U(\phi') \\
%\hat A_2 &\equiv \hat U^{\dagger}(\theta) \hat X \hat U(\theta), 
%&\hat A'_2 &\equiv \hat U^{\dagger}(\theta') \hat X \hat U(\theta')
%\end{align*}

Having defined $\Psi$, $\hat A_i$ and $\hat A'_i$, we are now in a position to evaluate $\mean{\mathcal {\hat B}}$. We essentially need terms like $ \mean{ \hat A_{1} \otimes \hat A_{2}}$. %$\left\langle\hat{U}^{\dagger}(\phi)\hat{X}\hat{U}(\phi)\otimes\hat{U}^{\dagger}(\theta)\hat{X}\hat{U}(\theta)\right\rangle $. 

If we assume $N\to\infty$, these can be evaluated easily to be the same as the spin case; $-\cos(\phi-\theta)$ (see \subref{c3}). Substituting for the angles as defined, we obtain $\mean{\mathcal {\hat B}} =2\sqrt{2}$; the maximum violation, in a completely different setup. If $N$ is not assumed to be arbitrarily large, then it can be shown that (see \subref{c5})
\[
\mean{\hat A_1 \otimes \hat A_2} = -\left( \frac{N-1}{N} \right)^2 \cos(\phi-\theta)
\]
which was somewhat expected. Thus obviously,
 \[
\mean{ \mathcal {\hat B} } =\left(\frac{N-1}{N}\right)^{2}2\sqrt{2}
\] 
If we ask for the smallest $N$ s.t. $\mean{ \mathcal {\hat B} } >2$, we get $N\ge7$; see figure \ref{chshViolation}.
...
To be consistent with out framework, we must show that the real functions $\mathcal W_{\hat A_i}$ and $\mathcal W_{\hat A'_i}$ are $\le 1$. To that end, we note that
...
\begin{eqnarray*}
\mathcal W_{\hat X (\delta)}(q,p) & = & \frac{1}{2}\int dq'e^{ipq'/\hbar} \bra{q-\frac{q'}{2}}\bigg( e^{-i\hat Z\phi/2}\\
 &  & e^{i\hat{p}\frac{L}{\hbar}}e^{i\hat Z\phi/2} + \text{h.c.} \bigg) \ket{q+\frac{q'}{2}}\\
 & = & \frac{1}{2}\left( e^{-iZ_{1}(q)\phi/2}e^{ip\frac{L}{\hbar}}e^{iZ_{2}(q)\phi/2}\right) + \text{h.c.}\\
 & = & \cos(pL/\hbar + Z_{2}(q)\phi)
\end{eqnarray*}
where $Z_{1/2}(q) \equiv Z[(q\mp\frac{L}{2})\text{mod}2L]$ and we used the fact that $Z(x)=-Z(x+L)$ (omitting the $\text{mod}2L$). Manifestly then, $\left| \mathcal W_{\hat A_i}\right|, \left| \mathcal W_{\hat A'_i}\right|  \le 1$ as was required.
