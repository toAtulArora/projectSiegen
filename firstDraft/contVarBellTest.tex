...
To construct the states, we start with defining $L\equiv$some length scale, $N\equiv$number of slits. Consider a state $\left|\varphi\right\rangle $ symmetric about $x=L/2$, with spread $\ll L$. Formally, $\left|\varphi\right\rangle \equiv\int dx\left\langle x-L/2|\varphi'\right\rangle \left|x\right\rangle $ where $\left|\varphi'\right\rangle $ s.t. (a) $\left\langle x|\varphi'\right\rangle =\left\langle -x|\varphi'\right\rangle $ and (b) it's spread is $\ll L$.
Next, we define $\left|\varphi_{n}\right\rangle \equiv\int dx\left\langle x-nL|\varphi\right\rangle \left|x\right\rangle $ and $I_{N}=\{-\frac{N}{2},\dots,\frac{N}{2}-1,\frac{N}{2}-1\}$, defined for even $N$ only (for simplicity). Finally we define the following to be analogues of $\ket{0}$ and $\ket{1}$;
...
\]
Explicitly, consider: (a) The first qubit is with Alic and the second with Bob. (b) Alice and Bob apply local unitaries $\hat U(\theta)\otimes \mathbb I$ and $\mathbb I \otimes \hat U(\phi)$ respectively and then both measure $\hat X$. (c) Alice can choose $\theta \in \{0,-\pi/2\}$ and Bob can choose $\phi \in \{3\pi/8,5\pi/4\}$.

Effectively, this amounts to defining \begin{align*}
\hat A_1 &\equiv \hat U^{\dagger}(\theta) \hat X \hat U(\theta),
&\hat A'_1 &\equiv \hat U^{\dagger}(\theta') \hat X \hat U(\theta') \\
\hat A_2 &\equiv \hat U^{\dagger}(\phi) \hat X \hat U(\phi), 
&\hat A'_2 &\equiv \hat U^{\dagger}(\phi') \hat X \hat U(\phi')
\end{align*}
The unitary $\hat U$ was introduced here to implement measurement along different directions in a plane, as is the case with the spins. To measure along different directions, within say the $x-y$ plane, one simply needs to rotate by appropriate angles using $\hat u(\phi) \equiv e^{i\hat{z}\phi/2}$. Correspondingly, here we define $\hat U$ by it's action on the relevant state. $\hat{U}(\phi)$ is defined to be s.t. 
\begin{eqnarray*}
...
\]
in close analogy with the spin case. It is suggestive from this form that $\hat{Z}$ must play the role of $\hat{z}$. We know that $\hat{X}$ is a function of $\hat{p}_{\text{mod}}$. It is thus natural to expect $\hat{Z}$ to be a function of $\hat{x}_{\text{mod}}$. From it's action, we also note that $\hat{Z}$ must differentiate between spatial wavefunctions $\left\langle x|\varphi\right\rangle $ and $\left\langle x-L|\varphi\right\rangle $. Thus the appropriate operator must be $\hat{x}_{\text{mod}2L}$ and not $\hat{x}_{\text{mod}L}$. This is supported by the fact that $[\hat{p}_{\text{mod}},\hat{x}_{\text{mod}L}]=0$. Now since the action of $\hat{Z}$ must work for arbitrary $\left|\varphi\right\rangle $ and $N$, we define
 \[
\hat{Z}\equiv Z(\hat{x}_{\text{mod}2L})
\]
where $Z$ is some appropriate function, so that $\hat{Z}\left|x\right\rangle =Z(\hat{x}_{\text{mod}2L})\left|x\right\rangle =Z(x_{\text{mod}2L})\left|x\right\rangle $. From the constraints on $\hat{Z}$, we conclude that for consistency,
...
To evaluate $\mean{\mathcal {\hat B}}$, we essentially need terms of the form $\left\langle \hat{U}^{\dagger}(\phi_{i})\hat{X}\hat{U}(\phi_{i})\otimes\hat{U}^{\dagger}(\theta_{i})\hat{X}\hat{U}(\theta_{i})\right\rangle $. If we assume $N\to\infty$, these can be evaluated easily to be the same as the spin case; $-\cos(\phi_{i}-\theta_{i})$ (see \subref{c3}). Substituting the angles as defined, we obtain $\left\langle \mathcal {\hat B} \right\rangle =2\sqrt{2}$; the maximum violation, in a completely different setup. If $N$ is not assumed to be arbitrarily large, then it can be shown that (see \subref{c5})
