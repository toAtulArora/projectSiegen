...
\date{May-July 2015}
\begin{abstract}
Local realism tests in phase space which fulfill the explicit assumptions underling Bell's theorem is of fundamental significant.
We construct a CHSH-Bell inequality in terms of continuum valued function of position and momentum variables known as modular variables.  This type of  observables are bounded in phase space and, thus the associate inequality hold for any state described by positive Wigner function serving as a valid probability distribution, and hence compatible with a model of local hidden variable. This conform with Bell conjecture that positive Wigner function does not reveal nonlocality. We construct a class of entangled states leading to the violation of the inequality, and thus truly demonstrate nonlocality in phase space. The states can be realized in interferometric setup though grating technique. The nonlocality is verified from spatial correlation data detected from the screens.
\end{abstract}
\maketitle
...
just as was with $\hat x$ and $\ket{\pm}$. In general however, the relation doesn't hold. Note that definition of $\left|\psi_{\pm}\right\rangle $ justifies the word `slit' in the definition of $N$, as is clear from the illustration. This construction also preserves the orthogonality relations from the spin case; $\left\langle \psi_{0}|\psi_{1}\right\rangle =0,\,\implies\left\langle \psi_{+}|\psi_{-}\right\rangle =0$. Analogously, we construct the entangled state 
 \[
\ket{\Psi} \equiv\frac{ \ket{\psi_{+}}_A\ket{\psi_{-}}_B - \ket{\psi_{-}}_A\ket{\psi_{+}}_B }{\sqrt{2}}
\]
In the spin case, a violation can be arrived at by measuring along different specific directions in a plane. To measure along different directions, within say the $x-y$ plane, one simply needs to rotate by appropriate angles, say \{$\phi$, $\theta$\}. This can be achieved by operating
$e^{i\hat{z}\phi/2}\otimes e^{i\hat{z}\theta/2}$ on the state before measurement. Correspondingly, here we define this operator by it's action on the relevant state. $\hat{U}(\phi)$ is defined to be s.t. 
\begin{eqnarray*}
...
$\left\langle \mathcal B \right\rangle $ as a function of $N$. To get a violation,
we need merely $7$ slits, while with $50$ we are almost at $2\sqrt{2}$.\label{chshViolation}}
\end{figure}


To evaluate the CHSH inequality, we again need terms of the form $\left\langle \hat{U}^{\dagger}(\phi_{i})\hat{X}\hat{U}(\phi_{i})\otimes\hat{U}^{\dagger}(\theta_{i})\hat{X}\hat{U}(\theta_{i})\right\rangle $, where $\theta \in \{0,-\pi/2\}$ and $\phi \in \{3\pi/8,5\pi/4\}$. If we assume $N\to\infty$, these can be evaluated easily to be the same as the spin case; $-\cos(\phi_{i}-\theta_{i})$. Thus, the same analysis as that of the spin case yields the maximum violation, $\left\langle \mathcal B \right\rangle =2\sqrt{2}$ which is the same maximal violation, in a completely different setup. If $N$ is not assumed to be arbitrarily large, then it can be shown that (see \subref{c5})
...
Thus obviously then, $\left\langle C\right\rangle =\left(\frac{N-1}{N}\right)^{2}2\sqrt{2}$. If we ask for the smallest $N$ s.t. $\left\langle C\right\rangle >2$, we get $N\ge7$; see figure \ref{chshViolation}.
