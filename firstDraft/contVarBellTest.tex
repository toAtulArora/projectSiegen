...
Eigenvalues of $\hat A_j$ 
$
|a_j|\leq 1$
which for bounded observables can be achieved trivially by rescaling. An example of this is the parity operator which was used in Ref [Ali].\\
...
\end{align*}
where $\bs q\equiv(q_1,q_2)$ and  $\bs p\equiv(p_1, p_2)$. \\
...
\[
\hat p_{\text{mod}\frac{h}{L}}\equiv \int p\text{mod}\frac{h}{L} \ket{p}\bra{p} dp
\]
...
The idea is to construct a Bell operator (see equation \ref{Belloperator}) in terms of $\hat X$ in which the different measurement settings are chosen by rotating it by suitable unitary operators.
...
Also, an analogy with the spin\footnote{Defn: $\left|0\right\rangle ,\,\left|1\right\rangle $ are s.t. $\hat{\sigma}_z\left|0\right\rangle =\left|0\right\rangle $
...
Now we define the following as analogues of $\ket{0}$ and $\ket{1}$;
...
Before proceeding further, we introduce a unitary operator $\hat U$ to implement different measurement settings. Again, motivated by the spins\footnote{where a measurement along a vector in the $x-y$ plane for example, maybe made by rotating by $e^{i\hat{\sigma}_z\phi/2}$ with $\phi$ appropriately chosen and then measuring $\hat{\sigma}_x$} we define $\hat U$ by its action 
\begin{align}
\label{eqn:U}
...
\end{figure}
The test is performed by considering two particles and their observers, Alice and Bob; they apply the aforesaid local unitary to define the setting, and then measure $\hat X$. %From above the two different parties, Alice and Bob, can chose their measurement settings performed on their respective particles using local unitaries of above form.
...
To prove (a), we use $\hat X(\theta)=\hat{X}e^{i\hat{Z}\theta}$, $e^{i\hat{Z}\theta}=\cos\theta+i\hat{Z}\sin\theta$ and the previous results, to arrive at 
\begin{align*}
[\hat X(\theta),\hat X(\theta')] &=
2i\sin(\theta'-\theta) \hat Z \hat X^2 \\
&=2i \hat Z \hat X^2 \neq 0
\end{align*}
where in the last equality holds when the angles are as defined. 

For showing (b) let's look at the Heisenberg equation of motion for the displacement operator 
\begin{align*}
\frac{d\hat X}{dt} &=i\hbar^{-1} [\hat Z,\hat X] \\  %\frac{\hat p^2}{2m} + \hat Z,\hat X]
  &=i\hbar^{-1} \left[ Z((\hat q)_{\text{mod}2L})-Z(\hat q_{\text{mod}2L}\pm L) \right] \hat X \\
  &=i\hbar^{-1} 2 \hat Z \hat X
\end{align*}
where $\hat Z$ is the potential. It follows that $\hat X (\theta)$ is essentially the $\hat X$ at some later time. This is already peculiar, that a step potential has any effect, given that the particle is never present at points where the potential is not constant. This is essentially the scalar AB effect. It is further strange that the equation of motion depends on potentials at two spatially distinct points. In the classical case however, we have
\[
\frac{dX}{dt} = \{ Z, X \} = -i\hbar^{-1}\frac{\partial Z}{\partial q} \sin(pL/\hbar)
\]
and since the slope of the step function is zero (= force), it means that classically $X(t)=X$ as expected. Manifestly, the non-commutativity of $\hat q$ and $\hat p$ results in $\hat X(t)\neq \hat X$ (as it follows a non-local equation of motion) which is the source of the violation.
