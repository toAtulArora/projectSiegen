...
The histogram discussed in sec [], will be given by 
\begin{align*}
\braket{p_1 p_2 | \Psi_f}&=\frac{1}{\sqrt 2} \tilde{\varphi}(p_1) \tilde{\varphi}(p_2) 
\sum_{n,m} e^{i(np_1 + mp_2)L/\hbar} \\
& \left[ -\cos(\delta') [(-)^m - (-)^n] + i \sin(\delta') [1 + (-)^{n+m}] \right]
\end{align*}
where $\tilde{\varphi}(p) \equiv \braket{p|\varphi}$ and $\delta' = (\phi - \theta)/2$.
