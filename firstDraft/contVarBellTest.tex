...
To be consistent with out framework, we must show that the observable in our case, given by $\hat{U}^{\dagger}(\phi)\hat{X}\hat{U}(\phi)$
is bounded in phase space. To that end, we note that
...
\begin{eqnarray*}
W_{1}(q,p) & = & \frac{1}{2\pi\hbar}\int dq'e^{ipq'/\hbar}\left\langle q-\frac{q'}{2}\right|e^{-i\hat Z\phi/2}\\
 &  & e^{i\hat{p}\frac{L}{\hbar}}e^{i\hat Z\phi/2}\left|q+\frac{q'}{2}\right\rangle \\
