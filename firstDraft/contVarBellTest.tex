...
Thus for particularly chosen angles, i.e., coplanar angles $\theta', \phi, \theta$ and $\phi'$ successively separated by $\pi/4$  we get
 \[
\ab{\mean{ \mathcal {\hat B} }} =\left(\frac{N-1}{N}\right)^{2}2\sqrt{2}
\] 
The violation, i.e.,  $\ab{\mean{ \mathcal {\hat B} }} >2$, achieved for $N > 6$; see figure \ref{chshViolation}. To interpret this, we must ensure that the assumptions of the framework are satisfied, viz. $\mathcal W_{\hat X(\phi)}\le 1$. To that end, we note that
\begin{eqnarray*}
...
&=2i \hat Z \hat X^2 \neq 0
\end{align*}
where in the last equality holds when the angles are as defined.  Classically this term not only vanishes, but the there can not be any difference between the two settings
...
\end{align*}
where $\hat Z$ is the potential. It follows that $\hat X (\theta)$ is essentially the $\hat X$ at some later time. This is already peculiar, because in our scenario the particle experiences constant potential exerting no force. That means classically the physical variables do not evolve, $X(t)=X(t_0)$ as expected. While for a quantum particle we consider here this is not the case. Modular variable can depend on the potential difference between spatially\emph{ separated} places \cite{PopescuNatPhys2010}. This is essentially nothing else but the scalar Aharonov-Bohm effect being exploited for realizing different settings in our setup. Manifestly, the non-commutativity of $\hat q$ and $\hat p$ results in $\hat X(t)\neq \hat X(t_0)$ (as it follows a non-local equation of motion) which is the source of the violation.
