...
qubits~\cite{Palacios-LaloyNatPhys2010}. All experimental observations confirmed the validity of quantum non-locality on this level. 
...
The outstanding challenge is, however, to formulate similar tests with truly phase space measurements where non-locality is inferred directly from observing, say, the spatial degree of freedom. %which maybe inferred directly from observation of only the spatial degree of freedom.
This can be considered as a natural extension to the macroscopic limit of local realism tests \cite{Reid}. Moreover, phase space is a natural concept in classical theory and it is equivalent to the state space. This is closer in spirit to the original EPR argument which uses phase space description to better address the reality and locality problems of quantum mechanics. % in such manifold. 
Notably, the Wigner function associated with the entangled state used in the EPR argument, the so called EPR state, is non-negative every where. Bell argued that the EPR states do not lead to a violation of the inequalities derived from local hidden variable assumptions. This was because non-negative Wigner functions serve as valid joint probability distributions over local \emph{hidden} positions and momenta. Thereby, in principle, a model of local hidden variable can be attributed to such states. Banaszek and Wodkiewicz \cite{Parity98}, however, showed that using particular measurements, namely parity, EPR states can reveal non-local features indicating that not only the state itself but the type of correlation measurements is also important in any local realism tests . This opens the discussion as to which measurements are good candidates for appropriately testing local hidden variable models \cite{Revzen}. The problem of constructing a ``macroscopic'' test of local hidden variable models hinges on choosing proper observables whose  Wigner representations satisfy the constraint imposed by the algebraic CHSH expression. The term ``macroscopic'' henceforth, will be used to refer to the measurement of a particle's phase-space coordinates.

The paper is organized as follows.
In Sec. we introduce our framework for the Bell test, aiming to use most classical-like variables and measurements. This motivates a macroscopic test of local realism \cite{Reid}. The observables we use are the so called modular variables \cite{Ahar69}. Recently, such variables have found applications in detecting certain CV entangled states ~\cite{Gneiting11}. Furthermore, they have been used for fundamental tests such as macroscopic realism \cite{AsadianLG}, contextuality \cite{Plastino, AsadianPS} and even the GHZ test \cite{Massar01}. This strongly suggests that modular variables can be used for local realism tests.  In Sec. we present an explicit example by constructing a Bell operator from modular variables, for which the violation is achieved only if the state is described by a negative Wigner function. In Sec. we proceed with identification of the relevant entangled state leading to the violation of the Bell inequality. Finally, in Sec. we show how the entire test can be implemented in a double grating setup.
...
In what follows we develop a new test of local realism which complies with the aforesaid argument of Bell. The central problem here is to construct a Bell-operator of the Clauser-HorneShimony-Holt (CHSH) form
\begin{equation}
\label{Belloperator}
\hat {\mathcal{B}} \equiv \hat A_1\otimes (\hat A_2+\hat A_2') +\hat A_1'\otimes(\hat A_2-\hat A_2')
\end{equation}
expressed in terms of suitable local continuous variable (CV) observables $\hat A$ which take a limited range of values to impose a well-defined classical bound. %be able to determine a well-defined classical bound. 
We therefore aim to construct a class of bounded observables of CV systems which have the following properties. Using proper rescaling for corresponding quantum eigenvalues of the observable we have 
$
|a_j|\leq 1$.
One example is the parity operator which was used in Ref [ ]. This is enough to construct a valid Bell operator with a well-defined classical bound. We however demand an extra constraint which is necessary for probing non-locality in phase space. We require that the observable $\hat A$ corresponds to a bounded c-number function in phase space obtained from the Wigner-Weyl correspondence ($q\leftrightarrow \hat q$, $p\leftrightarrow \hat p$), viz.
\[
...
\end{align*}
where $\bs q\equiv(q_1,q_2)$ and  $\bs p\equiv(p_1, p_2)$. Accordingly, the inequality should hold for any state described by valid (positive) probability distribution over phase space.
Therefore,
\begin{equation}
...
W_{\hat {\rho}} = \mathcal{W}_{\hat {\rho}}/2\pi \hbar$. Although formally valid, Bell inequality in terms of displaced parity operators void the said extra condition; their Wigner representations are given by delta functions which are unbounded in phase space. %This already voids the above version of inequality. 
...
For macroscopic test the measurement scheme used for evaluating the correlations should have a classical analog for better understanding the result in classical limit. The parity operator measurements are defined in terms of resolving the  consecutive eigen values  with no classical analog. Therefore we don't consider it a ``classical-like'' measurement.

\subsection{Phase space translation operators and modular variables }

One particular class of bounded observables can be constructed from the quantum mechanical space translation operator\footnote{Here $-L/\hbar$ causes a displacement by $L$.}, $e^{-i\hat p L/\hbar}$. As its name implies, this operator displaces a particle by a finite distance $L$, which in our case will be the distance between two adjacent slits. This operator is not an observable, therefore we define a symmetric combination   
\begin{align}
  \label{translation}
  \hat X \equiv\dfrac{e^{-i\hat p L/\hbar}+e^{i\hat p L/\hbar}}{2} =\cos(\hat p L/\hbar),
\end{align}
which is obviously a Hermitian operator and bounded by $\pm 1$. 
In fact the corresponding real function $\mathcal W_{\hat X}=\cos(pL/\hbar)$ is also manifestly  $\le 1$. Further, when $\hat X$ is operated on $\ket{p}$, then only the modular part of $p$ is relevant to the value of the operator. Thus we may define 
\[
\hat p_{\text{mod}\frac{2\pi\hbar}{L}}\equiv \int_{-\pi\hbar/L}^ {\pi\hbar/L}p \ket{p}\bra{p} dp
\]
and note that measuring $\hat p_{\text{mod}\frac{2\pi\hbar}{L}}$ is sufficient for obtaining the value of $\hat X$. Conversely, measuring $\hat X$ only yields $\hat p_{\text{mod}\frac{2\pi\hbar}{L}}$, not $\hat p$. Now, the idea is basically to construct a Bell operator, i.e. Eq. \ref{Belloperator}, in terms of $\hat X$ in which the different measurement settings are chosen by rotating it by suitable unitary operators.
...
Meanwhile, analogy with the spin\footnote{Defn: $\left|0\right\rangle ,\,\left|1\right\rangle $ are s.t. $\hat{\sigma}_z\left|0\right\rangle =\left|0\right\rangle $
...
} states, $\left|0\right\rangle ,\left|1\right\rangle ,\left|+\right\rangle ,\left|-\right\rangle $, will presumably be beneficial in constructing the relevant entangled state to achieve violation of the \emph{modular} Bell inequality for suitable choices of measurement settings.%\footnote{We emphasise that the kets we construct, will not be a countable superposition of eigenstates of $\hat p$ or $\hat q$ since these are highly idealized and strictly, not even a part of the Hilbert space.}. 
...
\bra{\psi_-} \hat X \ket{\psi_-} &= -\frac{N-1}{N}
\end{align*}
where $N \equiv 2M$ is supposed to be the number of `slits'.
Before proceeding further, we introduce a unitary operator $\hat U$ to implement different measurement settings. Again, motivated by the spins\footnote{where a measurement along a vector in the $x-y$ plane for example, maybe made by rotating by $e^{i\hat{\sigma}_z\phi/2}$ with $\phi$ appropriately chosen and then measuring $\hat{\sigma}_x$} we define $\hat U$ with effects 
\begin{align}
\label{eqn:U}
...
\]
where $\hat{Z}$ is s.t. it satisfies $\hat{Z}\left|\psi_{0}\right\rangle =\left|\psi_{0}\right\rangle $ and $\hat{Z}\left|\psi_{1}\right\rangle =-\left|\psi_{1}\right\rangle $. 
From its action, we note that $\hat{Z}$ must differentiate between spatial wavefunctions $\left\langle q|\varphi\right\rangle $ and $\left\langle q-L|\varphi\right\rangle $. It is thus natural to expect $\hat{Z}$ to be a function of  $\hat{q}_{\text{mod} 2L}$, i.e. 
$\hat{Z}\equiv Z(\hat{q}_{\text{mod}2L})$. From its action on position eigenstates, $Z(\hat{q}_{\text{mod}2L})\left|q\right\rangle =Z(q_{\text{mod}2L})\left|q\right\rangle $ we conclude that
\[
...
\end{figure}
From above the two different parties, Alice and Bob, can chose their measurement settings performed on their respective particles using local unitaries of above form.
...
\] 
The constraint $\ab{\mean{ \mathcal {\hat B} }} >2$, yields $N > 6$; see figure \ref{chshViolation}. To interpret this, we must ensure that the assumptions of the framework are satisfied, viz. $\mathcal W_{\hat X(\phi)}\le 1$. To that end, we note that
\begin{eqnarray*}
...
\subsection{Nonlocality and noncommutativity}
It is well recognized and it can be shown that there is a tight relation between nonlocality and noncommutativity of operators. This can be shown with
\begin{equation}
...
Using $\hat X(\theta)=\hat{X}e^{i\hat{Z}\theta}$, and $e^{i\hat{Z}\theta}=\cos\theta+i\hat{Z}\sin\theta$ and the previous results, it follows that 
\begin{eqnarray*}
...
We show that this scheme can be implemented using photons. We harness the two degrees of freedom of a photon, it's polarization and it's position to construct the required state. With a slightly modified setup, it is possible to do the same with spin and position
...
and the screens and $p_{z}$ is the $z$ component of momentum of the particle. For a photon, $p_{z}=h/\lambda$ while for a massive particle with mass $m$, $p_{z}=mD/T$, where $T$ is the time taken to arrive at the screen from the grating. The idea is simply that the momentum distribution at the grating can be recovered by observing the spatial distribution at the screen, sufficiently far away.
...
In the present work, we constructed a new Bell-operator in terms of phase space measurement via modular variables. In this particular scheme there was no possibility for bipartite system with  positive definite Wigner function, formally entangled or not, to yield a violation of the inequality. Therefore, a violation of the inequality truly demonstrated nonlocality in phase space as apposed to other approaches [ ]. The measurement observables were very simple and closer to the classical limit of measurements. The relevant entangled states used for achieving a violation of the inequality however required creation of multi-component superposition states through grating with enough number of slits. 
